...
def metodoGaussSeidel(equacoes, variaveis, precisao):
	...
	if(converge(equacoes, qtdEquacoes)):
		...
		for parada in range(0, precisao):
			for i in range(0, qtdEquacoes):
				#Iniciando substituições, substituir em qtdEquacoes - 2 variáveis 
				...
				for j in range(0, qtdEquacoes):
					...
					if(vetResultado[i].has(variaveis[j])):
						...
						vetResultado[i] = vetResultado[i].subs(variaveis[j], vetResultado[j])
...
def main():
	...
	while(True):
		...
		resultado = metodoGaussSeidel(equacoes, variaveis, precisao)
